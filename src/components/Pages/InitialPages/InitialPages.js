...
export default function Page1(props){
    ...
    console.log("sindhu",process.env.REACT_CLIENT_URL);
    useEffect(() => { fetch(process.env.REACT_CLIENT_URL).then((res) => res.json()).then(res => {
        console.log("boom", res);dispatch(assignJobCardDetails(res))
    })
    }, [])
   ...
   console.log("your active location length is"+selectedPlace.currentLocation.length);
   const skills=useSelector((state => state.showSkills));
    console.log('Your skills are'+skills.activeSkills);
   console.log(stepper.activeStepper + "i am stepper ");
   console.log("above is stepper value");
    const jobPlace=useSelector((state=>state.showJobLocation));
   skills.activeSkills.map(list=>(
   console.log('your skill is'+list.label)
   )); 
    console.log(jobPlace.jobLocation.length ,"is length ");
